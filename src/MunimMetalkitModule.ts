...
declare class MunimMetalkitModule extends NativeModule<MunimMetalkitModuleEvents> {
  ...
  createCanvas2D(width: number, height: number, pixelFormat?: MTLPixelFormat): Promise<Canvas2D>;
  clearCanvas2D(canvasId: string, backgroundColor?: Color2D): Promise<void>;
  
  // 2D Primitive Drawing
  drawLine2D(canvasId: string, start: Point2D, end: Point2D, style: LineStyle): Promise<void>;
  drawRectangle2D(canvasId: string, rect: Rectangle2D, style: FillStyle | LineStyle): Promise<void>;
  drawCircle2D(canvasId: string, circle: Circle2D, style: FillStyle | LineStyle): Promise<void>;
  drawEllipse2D(canvasId: string, ellipse: Ellipse2D, style: FillStyle | LineStyle): Promise<void>;
  drawPath2D(canvasId: string, path: Path2D, style: LineStyle): Promise<void>;
  
  // 2D Text Rendering
  drawText2D(canvasId: string, text: string, position: Point2D, style: TextStyle): Promise<void>;
  measureText2D(text: string, style: TextStyle): Promise<{ width: number; height: number }>;
  
  // 2D Image Operations
  drawImage2D(canvasId: string, imageTextureId: string, destination: Rectangle2D, source?: Rectangle2D): Promise<void>;
  compositeCanvas2D(canvasId: string, sourceCanvasId: string, operation: "source-over" | "source-in" | "source-out" | "source-atop" | "destination-over" | "destination-in" | "destination-out" | "destination-atop" | "xor" | "lighter" | "copy" | "multiply" | "screen" | "overlay" | "darken" | "lighten" | "color-dodge" | "color-burn" | "hard-light" | "soft-light" | "difference" | "exclusion"): Promise<void>;
  ...
  setLayerOpacity(canvasId: string, layerId: string, opacity: number): Promise<void>;
  setLayerBlendMode(canvasId: string, layerId: string, blendMode: string): Promise<void>;
  ...
  resizeCanvas2D(canvasId: string, width: number, height: number): Promise<void>;
  cropCanvas2D(canvasId: string, rect: Rectangle2D): Promise<void>;
  flipCanvas2D(canvasId: string, horizontal: boolean, vertical: boolean): Promise<void>;
  rotateCanvas2D(canvasId: string, angle: number): Promise<void>;
  
  // 2D Export and Import
  exportCanvas2D(canvasId: string, format: "png" | "jpg" | "webp"): Promise<ArrayBuffer>;
  importImageToCanvas2D(canvasId: string, imageData: ArrayBuffer, position: Point2D): Promise<void>;
  
  // 2D Utility Functions
  getCanvas2DPixel(canvasId: string, x: number, y: number): Promise<Color2D>;
  setCanvas2DPixel(canvasId: string, x: number, y: number, color: Color2D): Promise<void>;
